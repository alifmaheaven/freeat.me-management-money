<template>
  ...
  >
    <template #header>
      <div class="row">
        <div class="col-12 col-sm-12 col-md-6">
          <router-link to="/akun-transaksi/create">
    ...
    <!-- <Column field="nominal" header="Batas Nominal" sortable="true">
      <template #body="row">
        <span v-if="row.data.set_nominal == 1" class="badge badge-success">{{
          reformatToRupiah(row.data.nominal)
        }}</span>
        <span v-else class="badge badge-primary">Tidak ada batasan</span>
      </template>
    </Column> -->
    <Column field="action" header="Aksi">
      <template #body="row">
        <router-link :to="`/akun-transaksi/edit/${encrypter(row.data.id)}`">
  ...
  methods: {
    ...
    deleteRow(value) {
      this.$swal({
        ...
        if (result.value) {
          this.$store.commit("response_request");
          axios({ url: "api/account", data: value, method: "DELETE" })
            .then((result) => {
              this.$swal({
                icon: "success",
                title: "Sukses",
